...
public class CategoryController {
    ...
    public Result delete(@PathVariable String uuid) {
        if (!categoryService.selectByFatherUUID(uuid).isEmpty()) return Result.errorWithTitle("删除失败","待删除栏目包含子栏目");
        if (!articleService.selectByCategoryUUID(uuid).isEmpty()) return Result.errorWithTitle("删除失败","待删除栏目包含文章");
    ...
    public Result batchDelete(@RequestBody List<String> ids) {
        ...
        ids.forEach(id -> {
            if (!categoryService.selectByFatherUUID(id).isEmpty()) hasCategory.add(categoryService.selectById(id).getCategory());
            else if (!articleService.selectByCategoryUUID(id).isEmpty()) hasArticle.add(categoryService.selectById(id).getCategory());
