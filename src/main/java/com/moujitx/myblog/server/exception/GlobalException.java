...
public class GlobalException {
    ...
    public Result serviceException(DataIntegrityViolationException e) {
        ...
        if (msg.contains("Duplicate entry"))
            content = msg.split("\\.")[1].split("'")[0] + "重复";
        else if (msg.contains("Cannot delete or update a parent row: a foreign key constraint fails"))
            content = "其他表中存在值与被删除项绑定";
        else if (msg.contains("Cannot add or update a child row: a foreign key constraint fails"))
            content = "外键值不存在";
